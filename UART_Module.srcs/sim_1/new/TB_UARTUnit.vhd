...
architecture TESTBENCH of TB_UARTUnit is
    ...
    signal tb_received_data : std_logic_vector(7 downto 0);
    signal tb_new_data_received : std_logic;
    signal tb_RX_pin : std_logic;
    constant tbase : time := 83 ns;
begin
    COMP: UART_Unit generic map(12000000, 9600) port map(tb_clk, tb_rst, tb_send_data, tb_write_en, tb_full, tb_TX_Pin, tb_received_data, tb_new_data_received, tb_RX_pin);
    ...
    tb_send_data <= "01000000";
    tb_write_en <= '1';
    tb_RX_pin <= '1';
    -- Verified in practical use with logic analyzer
